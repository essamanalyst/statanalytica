...
// Storage keys
const STORAGE_KEYS = {
  DATASET: 'statanalytica_dataset',
  DATASET_META: 'statanalytica_dataset_meta',
  ACTIVE_TAB: 'statanalytica_active_tab',
};

// Helper functions for localStorage with compression for large data
const saveToStorage = (key: string, data: any) => {
  try {
    ...
    if (size > 4 * 1024 * 1024) {
      console.warn('Data too large for localStorage, saving metadata only');
      ...
      if (data && data.rows) {
        ...
        localStorage.setItem(STORAGE_KEYS.DATASET_META, JSON.stringify(metadata));
      }
      return false;
    }
    ...
    console.log(`Data saved to localStorage: ${(size / 1024).toFixed(2)} KB`);
    return true;
  } catch (error: any) {
    console.warn('Failed to save to localStorage:', error);
    // If quota exceeded, try to save just metadata
    if (error.name === 'QuotaExceededError' || error.code === 22) {
      console.warn('Storage quota exceeded, clearing old data');
      try {
        localStorage.removeItem(key);
        // Try saving smaller version
        if (data && data.rows && data.rows.length > 1000) {
          const smallerData = {
            ...data,
          ...
          console.log('Saved partial data (first 1000 rows)');
          return true;
        }
      } catch (e) {
        console.error('Could not save even partial data:', e);
      }
    }
    return false;
  }
...
const loadFromStorage = <T,>(key: string, defaultValue: T): T => {
  try {
    ...
    if (stored) {
      ...
      if (timestamp) {
        console.log(`Data loaded from localStorage (saved at: ${timestamp})`);
      }
      return parsed;
    }
  } catch (error) {
...
const removeFromStorage = (key: string) => {
  try {
    ...
    localStorage.removeItem(STORAGE_KEYS.DATASET_META);
  } catch (error) {
...
// Use it to log storage status
if (typeof window !== 'undefined') {
  console.log('localStorage available:', checkStorageAvailability());
}

export function App() {
  const { t, language, isRTL, dir } = useLanguage();
  const { notifyDataLoaded } = useNotification();
  const { isDark, toggleTheme } = useTheme();
  
  // Load initial state from localStorage
  const [activeTab, setActiveTab] = useState<string>(() => 
    loadFromStorage(STORAGE_KEYS.ACTIVE_TAB, 'upload')
  );
  const [dataset, setDataset] = useState<Dataset | null>(() => 
    loadFromStorage(STORAGE_KEYS.DATASET, null)
  );
  ...
  const [showDataPersistNotice, setShowDataPersistNotice] = useState(false);

  // Save dataset to localStorage whenever it changes
  useEffect(() => {
    if (dataset && dataset.rows && dataset.rows.length > 0) {
      const saved = saveToStorage(STORAGE_KEYS.DATASET, dataset);
      if (saved) {
        console.log('Dataset saved successfully:', dataset.rowCount, 'rows');
      }
    }
  }, [dataset]);
  ...
  // Show notice when data is restored from localStorage
  useEffect(() => {
    try {
      const storedDataset = loadFromStorage<Dataset | null>(STORAGE_KEYS.DATASET, null);
      if (storedDataset && storedDataset.rows && storedDataset.rows.length > 0) {
        console.log('Data restored from localStorage:', storedDataset.rowCount, 'rows');
        setShowDataPersistNotice(true);
        const timer = setTimeout(() => setShowDataPersistNotice(false), 5000);
  ...
  const clearStoredData = useCallback(() => {
    removeFromStorage(STORAGE_KEYS.DATASET);
    setDataset(null);
    setActiveTab('upload');
  }, []);
  ...
  const handleDataLoaded = useCallback((data: any[], columns: string[]) => {
    ...
    // إشعار المستخدم بنجاح تحميل البيانات
    notifyDataLoaded(data.length, columns.length);
  }, [language, notifyDataLoaded]);
  ...
  return (
    <div className={cn('flex h-screen', isDark ? 'dark bg-gray-900' : 'bg-gray-100')} dir={dir}>
      {/* Sidebar */}
      <Sidebar 
        activeTab={activeTab} 
              ...
              {dataset && (
                <div className={cn(
                  ...
                  <button
                    ...
                    className="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 px-2 py-1 rounded-lg transition-all flex items-center gap-1"
            ...
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className={cn(
                  ...
                  isDark ? "text-yellow-400 hover:text-yellow-300 hover:bg-gray-700" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"
                )}
                title={isDark ? (language === 'ar' ? 'الوضع الفاتح' : 'Light Mode') : (language === 'ar' ? 'الوضع الداكن' : 'Dark Mode')}
              >
      ...
      {/* Data Persist Notice */}
      {showDataPersistNotice && dataset && (
        <div className={cn(
          "fixed bottom-20 z-50 bg-gradient-to-r from-green-500 to-emerald-600 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4 border border-green-400",
        ...
        >
          ...
          <div>
            <p className="font-bold text-lg flex items-center gap-2">
              ✓ {language === 'ar' ? 'تم استعادة البيانات بنجاح' : 'Data Restored Successfully'}
            </p>
            <p className="text-sm opacity-90 mt-1">
              {language === 'ar' 
                ? `البيانات المحفوظة: ${dataset.rowCount.toLocaleString()} صف × ${dataset.columnCount} عمود`
                : `Saved data: ${dataset.rowCount.toLocaleString()} rows × ${dataset.columnCount} columns`
              }
            </p>
            <p className="text-xs opacity-75 mt-1">
              {language === 'ar' 
                ? 'البيانات محفوظة تلقائياً في المتصفح'
                : 'Data is automatically saved in browser'
              }
            </p>
          </div>
          <button 
            onClick={() => setShowDataPersistNotice(false)}
            className="p-2 hover:bg-white/20 rounded-lg transition-all absolute top-2 right-2"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
